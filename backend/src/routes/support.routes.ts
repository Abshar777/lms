 ...
 *     GET    /support/admin        — all tickets (?status=&search=)
 *     GET    /support/admin/stats  — status counts
 *     GET    /support/:id          — view any ticket
...
import { authenticate, authenticateAdmin, authenticateAny, requireRole } from '@/middleware/auth.middleware.ts'
...
router.get('/admin/stats',  authenticateAdmin, requireRole('admin'), ctrl.stats)
router.get('/admin',        authenticateAdmin, requireRole('admin'), ctrl.listAll)
router.patch('/:id/status', authenticateAdmin, requireRole('admin'), validate(statusSchema), ctrl.setStatus)
...
router.get('/:id',           authenticateAny, ctrl.getOne)
router.post('/:id/messages', authenticateAny, validate(messageSchema), ctrl.addMessage)
