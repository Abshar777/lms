...
import {
  ...
  Settings, ChevronLeft, LogOut, Flame, Map, X,
} from 'lucide-react'
            ...
            <motion.div key={item.href} custom={navItems.length + i} variants={itemVariants} initial="hidden" animate="show">
              <Link href={item.href} onClick={onClose}>
                <div className="flex items-center gap-3 rounded-xl px-3 py-2.5 transition-colors hover:bg-orange-50"
                  style={{ color: '#9CA3AF' }} title={collapsed ? item.label : undefined}>
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="fixed left-0 top-0 z-40 hidden h-screen flex-col overflow-hidden bg-white lg:flex"
        ...
        {mobileNavOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed left-0 top-0 z-50 flex h-screen w-[280px] flex-col overflow-hidden bg-white lg:hidden"
              style={{ borderRight: '1px solid #E4E7ED' }}
            >
